...
for (const cat of occupationList){
  promises.push(d3.text(`groups/liste_${cat}.csv`));
}
...
for (const cat of regionList){
  promises.push(d3.text(`groups/${cat}.csv`));
}
...
function updateViz(){
  ...
  graphSim
  .jsonUrl(`output/graph_coquotation_unfiltered_all_1_filt_10_${journal}.json`)
